...
function transformDateValue (field, item, options) {
	var target = field._path.parts[0];
...
function getCSVData (item, options) {
	...
	if (options.fields) {
		...
		console.log(this.fields["dateJoin"]._path.parts[0]);
		options.fields.forEach(function (path) {
			...
			if(field.type == "date"){
				data[path] = transformDateValue(field, item, options);
				return;
			}
			else if (field.type !== 'relationship' || !options.expandRelationshipFields) {
