...
module.exports = function(req, res) {
	...
	if (req.user.isSuperAdmin == false) {
		//console.log(req.list.model);
		if (req.list.model.modelName == "Event") {
			if (req.user.country != "All") {
				assign(
					where,
					req.list.addFiltersToQuery({
						country: { mode: "contains", inverted: false, value: req.user.country }
					})
				);
			}
		}
